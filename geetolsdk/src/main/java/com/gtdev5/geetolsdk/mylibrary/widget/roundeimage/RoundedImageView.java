...
package com.gtdev5.geetolsdk.mylibrary.widget.roundeimage;
...
public class RoundedImageView extends android.support.v7.widget.AppCompatImageView {
    ...
    private void updateAttrs(Drawable drawable, ScaleType scaleType) {
        ...
        if (drawable instanceof RoundedDrawable) {
            ((RoundedDrawable) drawable).setScaleType(scaleType).setBorderWidth(mBorderWidth).setBorderColor(mBorderColor).setOval(mIsOval).setTileModeX(mTileModeX).setTileModeY(mTileModeY);

            if (mCornerRadii != null) {
                ((RoundedDrawable) drawable).setCornerRadius(mCornerRadii[Corner.TOP_LEFT], mCornerRadii[Corner.TOP_RIGHT], mCornerRadii[Corner.BOTTOM_RIGHT], mCornerRadii[Corner.BOTTOM_LEFT]);
            }
